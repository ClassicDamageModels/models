...
var getAttackTable = function getAttackTable(type, rawStats, weapons) {
  ...
  return ['mh', 'oh'].map(function (weapon) {
    var DUALWIELDING = !!(weapons[0] && weapons[1]);

    var stats = _lodash.default.mergeWith({}, rawStats, bonus, _misc.addNumbers); // Base miss chance against level 73 is 9% (+19% if dualwielding)
    ...
    var DODGE_CHANCE = _lodash.default.clamp(6.5 - (weapon === 'mh' ? Math.floor(stats.expertise_mh) : Math.floor(stats.expertise_oh)) * 0.25, 0, 100);
